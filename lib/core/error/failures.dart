...
sealed class Failure with _$Failure {
  const factory Failure.server([@Default('Server Failure') String message]) =
      ServerFailure;
  ...
  const factory Failure.cache([@Default('Cache Error') String message]) =
      CacheFailure;
  const factory Failure.location([@Default('Location Error') String message]) =
      LocationFailure;
  const factory Failure.biometric([
    @Default('Biometric Authentication Error') String message,
  ]) = BiometricFailure;
  ...
  const factory Failure.conflict(
    String code, [
    @Default('Request conflict') String message,
