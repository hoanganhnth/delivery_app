...
void main() {
  ...
  test(
    'reads user orders through canonical /api/orders/my-orders path',
    () async {
      adapter.onGet(
        '/orders/my-orders',
  ...
  test(
    'submits checkout preview through canonical /api/orders/checkout-preview path',
    () async {
      final request = CheckoutPreviewRequest(
        ...
        couponCode: null,
        items: [const CheckoutPreviewItemRequest(menuItemId: 1, quantity: 1)],
      );
  ...
  test(
    'creates and cancels order through canonical /api/orders path',
    () async {
      ...
      final created = await service.createOrderWithDto(
        createRequest,
      ...
      final cancelled = await service.cancelOrder(101, {
        'reason': 'Changed mind',
      });
