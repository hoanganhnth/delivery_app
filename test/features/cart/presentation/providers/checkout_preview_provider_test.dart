...
import 'package:delivery_app/features/cart/di/checkout_preview_provider.dart';
...
void main() {
  test(
    'checkout preview is single-submit and stores canonical server totals',
    () async {
  ...
  test(
    'mismatched server items fail closed instead of enabling checkout',
    () async {
      ...
      final service = _FakeOrderApiService(
        (_) async => BaseResponseDto(status: 1, message: 'ok', data: malformed),
      );
      final container = _container(service);
      addTearDown(container.dispose);
      container.listen(checkoutPreviewProvider, (_, _) {});
      ...
      expect(container.read(checkoutPreviewProvider).hasError, isTrue);
      expect(
        container.read(checkoutPreviewProvider).error,
        isA<FormatException>(),
      );
    },
  );
