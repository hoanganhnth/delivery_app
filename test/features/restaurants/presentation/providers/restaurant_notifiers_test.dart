...
void main() {
  group('restaurant catalog and detail providers', () {
    test(
      'loads catalog, exposes loading, then retries a backend failure',
      () async {
        ...
        expect(
          container.read(restaurantsProvider).errorMessage,
          'Mất kết nối danh sách',
        );
    ...
    test(
      'searches by query, restores the catalog for blank input and limits featured rows',
      () async {
        final repository = _FakeRestaurantRepository();
        final container = _container(repository);
        addTearDown(container.dispose);
        ...
        repository.searchResult = Right([
          buildRestaurant(name: 'Bếp tìm kiếm'),
        ]);
        await notifier.searchRestaurants('  cơm  ');
        expect(repository.lastSearchQuery, '  cơm  ');
        expect(
          container.read(restaurantsProvider).restaurants.single.name,
          'Bếp tìm kiếm',
        );

        repository.restaurantsResult = Right([
          buildRestaurant(name: 'Danh sách gốc'),
        ]);
    ...
    test(
      'keeps restaurant facts when menu load fails and retries menu only',
      () async {
        final repository = _FakeRestaurantRepository();
        final container = _container(repository);
        addTearDown(container.dispose);
